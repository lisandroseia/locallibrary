...
exports.book_list = asyncHandler(async (req, res, next) => {
  ...
  .populate("author")
  .exec()
  res.render("books_list", {title: "Books list", book_list: allBooks})
});
...
exports.book_detail = asyncHandler(async (req, res, next) => {
  ...
  const [book, bookInstances] = await Promise.all([
    Book.findById(req.params.id).populate("author").populate({path: "genre", model: Genre}).exec(),
...
exports.book_delete_get = asyncHandler(async (req, res, next) => {
  ...
  const [book, allBookInstances] = await Promise.all([
    ...
    BookInstance.find({book: req.params.id}, "book status").populate("book").exec()
  ])
